...
double ** current_coordinate() {
    ...
    ifs.open("../current_coordinate.txt",std::ios::out | std::ios::in);
...
bool iscrash(double *coordinate, double* coordinate2) {
    bool iscrash = 0;
    Line l1, l2;
...
bool crash(double *coordinate, double *coordinate2, bool can_crash) {
...
void coordinate_code() {
    ...
    for(i = 0; coordinate[i][0] > 0; i++){
    str2 = QString("var start%1 = {lat: %2, lng: %3};\n").arg(i + 1).arg(coordinate[i][4]).arg(coordinate[i][5]);
    out << str2;
